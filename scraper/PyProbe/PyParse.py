...
from httpx import HTTPStatusError, RequestError, ConnectError, TimeoutException, NetworkError, InvalidURL, TooManyRedirects
import logging
import random
import asyncio
    ...
    async with httpx.AsyncClient() as client:
        for attempt in range(max_retries):
            ...
            try:
                ...
                error_message = "A network error occurred. Please check your network settings."
                logging.error(f"Attempt {attempt + 1}/{max_retries} for {url}: {error_message}")
                ...
                error_message = "Too many redirects. Please check the URL or try again later."
                logging.error(f"Attempt {attempt + 1}/{max_retries} for {url}: {error_message}")
                ...
                error_message = "The URL provided is invalid. Please check the URL format."
                logging.error(f"Attempt {attempt + 1}/{max_retries} for {url}: {error_message}")
