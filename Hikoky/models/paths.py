    ...
    def get_link(cls, source, path):
        ...
        if link:
            ...
            raise HTTPException(
                status_code=404,
                detail={
                    "error": "Manga not available. Please check the path validity."
                },
...
class PathChapter(Base):
    ...
    def get_link(cls, source, path_manga, path_chapter):
        with Database.get_session() as session:
            link = (
                session.query(PathChapter.link)
                .filter(
        ...
        if link:
            ...
            raise HTTPException(
                status_code=404,
                detail={
                    "error": "Chapter not available. Please check the path validity."
                },
